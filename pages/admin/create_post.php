...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // 1. Buat Slug dari Judul (untuk nama folder juga)
    ...
    // 2. Tentukan Folder Tujuan Berdasarkan Slug
    $upload_root = "../../assets/uploads/";
    $project_folder = $upload_root . $slug . "/"; // Contoh: ../../assets/uploads/rina-dika-wedding/
    
    // 3. Buat Folder jika belum ada
    if (!file_exists($project_folder)) {
        // 0777 = permission full, true = recursive creation
    ...
    if (!empty($_FILES["thumbnail"]["name"])) {
        ...
        $db_image_path = "assets/uploads/" . $slug . "/" . $filename;
...
<!DOCTYPE html>
<!-- (SISA KODE HTML SAMA PERSIS SEPERTI SEBELUMNYA) -->
<!-- Copy HTML dari create_post.php sebelumnya di sini -->
            ...
            <div class="max-w-[1200px] mx-auto flex flex-col gap-6">
                <form action="" method="POST" enctype="multipart/form-data">
                    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
                        ...
                        <div class="flex items-center gap-3">
                            <a href="admin_portfolio.php" class="px-5 py-2.5 rounded-lg border border-[#cfd7e7] bg-white text-[#4c669a] font-semibold text-sm hover:bg-[#f3f4f6] hover:text-[#0d121b] transition-colors shadow-sm">Batal</a>
                            <button type="submit" class="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-primary hover:bg-primary-hover text-white font-bold text-sm shadow-sm shadow-primary/30 transition-colors">
                                <span class="material-symbols-outlined text-[20px]">save</span> Simpan & Publish
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                            ...
                            <div class="bg-white p-6 rounded-xl border border-[#cfd7e7] shadow-sm">
                                ...
                                <div class="space-y-5">
                                    ...
                                    <div>
                                        ...
                                        <textarea class="block w-full rounded-lg border-[#cfd7e7] bg-[#f8fafc] text-[#0d121b] placeholder-[#94a3b8] focus:border-primary focus:ring-primary focus:bg-white transition-all sm:text-sm p-2.5 shadow-sm" name="description" placeholder="Tulis deskripsi..." rows="8"></textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="bg-white p-6 rounded-xl border border-[#cfd7e7] shadow-sm">
                        ...
                        <div class="lg:col-span-1 flex flex-col gap-6">
                            ...
                            <div class="bg-white p-6 rounded-xl border border-[#cfd7e7] shadow-sm">
                                ...
                                <div class="mb-4">
                                    <label class="block text-sm font-medium text-[#0d121b] mb-1.5">Gambar Utama</label>
                                    <label for="thumbnail-upload" class="border-2 border-dashed border-[#cfd7e7] rounded-xl p-6 flex flex-col items-center justify-center text-center bg-[#f8fafc] hover:bg-[#f1f5f9] hover:border-primary/50 transition-all cursor-pointer group h-48 relative">
                                        ...
                                        <p class="text-[10px] text-[#94a3b8] mt-2 uppercase">PNG, JPG, WEBP</p>
